...
# El dataset tiene 250 000 filas. Para que el tuning sea tratable en un
# equipo de escritorio, trabajamos con una muestra estratificada de 10 000
# observaciones. Si tu equipo tiene suficiente RAM y tiempo, aumenta n_muestra.
n_muestra <- 10000

datos_full <- read_excel("datossalarios.xlsx", sheet = "salarios")

cat(sprintf("\nDataset completo: %d filas × %d columnas\n",
            nrow(datos_full), ncol(datos_full)))

# Preprocesamiento: codificar variables categóricas a entero
datos_full <- datos_full %>%
  mutate(
    titulo_profesional = as.integer(factor(titulo_profesional)),
    nivel_educativo    = as.integer(factor(
      nivel_educativo,
      levels = c("High School","Diploma","Bachelor","Master","PhD"))),
    industria          = as.integer(factor(industria)),
    tamaño_empresa     = as.integer(factor(
      tamaño_empresa,
      levels = c("Startup","Small","Medium","Large","Enterprise"))),
    ubicacion          = as.integer(factor(ubicacion)),          # nota: columna sin tilde
    trabajo_remoto     = as.integer(factor(
      trabajo_remoto, levels = c("No","Hybrid","Yes")))
  )

# Renombrar columna con tilde para evitar problemas
names(datos_full)[names(datos_full) == "ubicación"] <- "ubicacion"

set.seed(SEED)
datos <- datos_full %>% slice_sample(n = n_muestra)

cat(sprintf("Muestra de trabajo: %d obs\n", nrow(datos)))
...
ggplot(datos, aes(salario)) +
  geom_histogram(bins = 40, fill = "darkorange", color = "white", alpha = .85) +
  scale_x_continuous(labels = scales::comma) +
  labs(title = "Distribución de salario (variable objetivo)",
       x = "Salario anual (USD)", y = "Frecuencia") +
  theme_bw()

# Salario por título profesional
datos_full %>%
  slice_sample(n = 5000) %>%
  mutate(titulo_profesional = factor(titulo_profesional)) %>%
  ggplot(aes(reorder(factor(titulo_profesional), salario, median), salario)) +
  geom_boxplot(fill = "darkorange", alpha = .7, outlier.size = .5) +
  scale_y_continuous(labels = scales::comma) +
  coord_flip() +
  labs(title = "Salario por título profesional",
       x = NULL, y = "Salario (USD)") +
  theme_bw()
...
set.seed(SEED)
svm_lineal <- svm(salario ~ ., data = data_train,
                  kernel = "linear",
                  cost   = 1,
                  scale  = TRUE)
...
cat(sprintf("SVM Lineal → RMSE: %.1f | MAE: %.1f\n",
...
set.seed(SEED)
svm_poly <- svm(salario ~ ., data = data_train,
                kernel = "polynomial", degree = best_d,
                cost = 1, gamma = 0.1, coef0 = 0,
                scale = TRUE)
pred_poly <- predict(svm_poly, newdata = data_test)

cat(sprintf("SVM Poly (d=%d) → RMSE: %.1f | MAE: %.1f\n",
            best_d,
...
set.seed(SEED)
svm_radial_base <- svm(salario ~ ., data = data_train,
                       kernel = "radial",
                       cost  = 1,
...
cat(sprintf("SVM Radial base → RMSE: %.1f | MAE: %.1f | SV: %d\n",
...
p_gamma <- ggplot(df_gamma, aes(log10(gamma), RMSE)) +
  ...
  scale_y_continuous(labels = scales::comma) +
  labs(title = "SVM Radial: RMSE en test según γ (C=1) — salario",
       x = "log10(γ)", y = "RMSE (USD)") +
  theme_bw()
print(p_gamma)
...
cat("(puede tardar 3-8 minutos con 8 000 obs de entrenamiento)\n")
...
# e1071::tune() reporta MSE en regresión
cat(sprintf("Mejor C=%.3f | Mejor gamma=%.4f | MSE CV=%.1f (RMSE≈%.1f USD)\n",
...
plot(tune_result, main = "e1071::tune() — Error CV por C y γ (salario)")

svm_radial_tuned  <- tune_result$best.model
pred_radial_tuned <- predict(svm_radial_tuned, newdata = data_test)

cat(sprintf("SVM Radial tuned (e1071) → RMSE: %.1f | MAE: %.1f | SV: %d\n",
...
cat("(puede tardar 5-10 minutos)\n")

receta_svm <- recipe(salario ~ ., data = data_train) %>%
...
set.seed(SEED)
cv_folds_svm <- vfold_cv(data_train, v = 5, repeats = 2, strata = salario)

set.seed(SEED)
svm_grid <- grid_latin_hypercube(
  cost(range = c(-4, 7)),          # log2: ~0.06 – 128
...
cat(sprintf("SVM Radial (tidymodels) → RMSE: %.1f | MAE: %.1f\n",
...
comparativa_wide <- comparativa_svm %>%
  mutate(texto = sprintf("%.1f [%.1f, %.1f]", est, lower, upper)) %>%
...
comparativa_svm %>%
  ...
  scale_y_continuous(labels = scales::comma) +
  coord_flip() +
  labs(x = NULL, y = "RMSE en test (IC 95% Bootstrap, USD)",
       title = "Comparativa de variantes SVM — salario") +
  theme_bw()
...
p_a <- ggplot(data.frame(y_test, mejor_pred),
  ...
  geom_point(alpha = .4, color = "darkorange", size = .8) +
  geom_abline(slope = 1, intercept = 0, color = "black") +
  scale_x_continuous(labels = scales::comma) +
  scale_y_continuous(labels = scales::comma) +
  labs(x = "Predicho (USD)", y = "Real (USD)",
...
p_b <- ggplot(data.frame(mejor_pred, residuos),
  ...
  scale_x_continuous(labels = scales::comma) +
  scale_y_continuous(labels = scales::comma) +
  labs(x = "Predicho (USD)", y = "Residuo (USD)",
       title = "Residuos vs Predicho") + theme_bw()

p_c <- ggplot(data.frame(residuos), aes(sample = residuos)) +
  stat_qq(color = "darkorange", size = .8) + stat_qq_line(color = "black") +
...
set.seed(SEED)
idx_shap  <- sample(length(residuos), min(length(residuos), 5000))
shapiro_p <- shapiro.test(residuos[idx_shap])$p.value
t_p       <- t.test(residuos)$p.value
cat(sprintf("Residuos → p Shapiro (n=%d)=%.4f | p t.test(mu=0)=%.4f\n",
            length(idx_shap), shapiro_p, t_p))
...
vars_pred     <- setdiff(names(data_train), "salario")
mtry_grid_rf  <- 2:length(vars_pred)
oob_errors_rf <- sapply(mtry_grid_rf, function(m) {
  set.seed(SEED)
  rf <- randomForest(salario ~ ., data = data_train, mtry = m, ntree = 300)
  tail(rf$mse, 1)
})
...
set.seed(SEED)
modelo_rf_comp <- randomForest(salario ~ ., data = data_train,
                               mtry = mtry_opt_rf, ntree = 300,
                               nodesize = 5, importance = TRUE)
pred_rf_comp <- predict(modelo_rf_comp, newdata = data_test)

X_train_xgb <- data_train %>% select(-salario) %>% data.matrix()
X_test_xgb  <- data_test  %>% select(-salario) %>% data.matrix()
...
comparativa_total %>%
  ...
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.25) +
  scale_y_continuous(labels = scales::comma) +
  scale_fill_manual(values = c("SVM"          = "darkorange",
                               "RandomForest"  = "seagreen",
                               "XGBoost"       = "steelblue")) +
  coord_flip() +
  labs(x = NULL, y = "RMSE en test (IC 95% Bootstrap, USD)",
       title = "SVM vs Random Forest vs XGBoost — salario",
       fill = "Familia") +
...
ggplot(datos_tipo, aes(salario, fill = tipo)) +
  geom_histogram(bins = 40, position = "identity", alpha = 0.55) +
  scale_x_continuous(labels = scales::comma) +
  scale_fill_manual(values = c("Vector de Soporte" = "darkorange",
                               "No Soporte"        = "steelblue")) +
  labs(title = "Distribución de salario: Vectores de Soporte vs Resto",
       subtitle = "Los SV se concentran donde la predicción es más incierta",
       x = "Salario (USD)", y = "Frecuencia", fill = "") +
  theme_bw() + theme(legend.position = "bottom")

cat("\nEstadísticos salario — Vectores de Soporte:\n")
print(summary(sv_data$salario))
cat("Estadísticos salario — Resto de observaciones:\n")
print(summary(no_sv$salario))
...
p_sv2 <- ggplot(df_sv_cost, aes(log10(C), RMSE)) +
  ...
  scale_y_continuous(labels = scales::comma) +
  labs(title = "RMSE en test según C — salario",
       x = "log10(C)", y = "RMSE (USD)") +
  theme_bw()
...
=================================================================
  ...
  Base de datos: datossalarios.xlsx  (muestra n=10 000)
  Variable objetivo: salario (USD, cuantitativa continua)
=================================================================\n")
